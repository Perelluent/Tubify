...
public class ApiClient {
    ...
    private HttpRequest.Builder requestBuilder(String path, String jwt) {
        ...
        if (jwt != null && !jwt.isBlank()) b.header("Authorization", "Bearer " + jwt);
        return b;
    }
    ...
    public Optional<String> login(String email, String password) throws Exception {
        ...
        if (resp.statusCode() / 100 == 2) {
            ...
            else if (node.has("access_token")) token = node.get("access_token").asText();
    ...
    public Usuari getMe(String jwt) throws Exception {
        ...
        if (r.statusCode() / 100 == 2) return mapper.readValue(r.body(), Usuari.class);
    ...
    public String getNickName(int id, String jwt) throws Exception {
        ...
        if (r.statusCode() / 100 == 2) {
            ...
            try {
                ...
                if (node.isTextual()) return node.asText(); // "Mike"
                if (node.has("nickName")) return node.get("nickName").asText();
                if (node.has("nickname")) return node.get("nickname").asText();
    ...
    public List<Media> getAllMedia(String jwt) throws Exception {
        ...
        HttpResponse<String> r = client.send(req, HttpResponse.BodyHandlers.ofString());
        if (r.statusCode() / 100 == 2) return mapper.readValue(r.body(), new TypeReference<List<Media>>() {});
    ...
    public String listBlobs(String container, String jwt) throws Exception {
        String path = "/api/files" + (container == null ? "" : "?container=" + URLEncoder.encode(container, "UTF-8"));
    ...
    public void download(int id, String container, File destFile, String jwt) throws Exception {
        String path = "/api/files/" + id + (container == null ? "" : "?container=" + URLEncoder.encode(container, "UTF-8"));
        HttpRequest req = requestBuilder(path, jwt).GET().build();
    ...
    public String uploadFileMultipart(File file, String downloadedFromUrl, String container, String jwt) throws Exception {
        String boundary = "----JavaClientBoundary" + System.currentTimeMillis();
        Map<String, String> fields = new HashMap<>();
        if (downloadedFromUrl != null) fields.put("downloadedFromUrl", downloadedFromUrl);
        if (container != null) fields.put("container", container);
